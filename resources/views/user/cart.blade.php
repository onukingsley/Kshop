    ...
    <div class="container pb-5 mb-2 mb-md-4">
        <div class="row">
            ...
            <section class="col-lg-8">
                ...
                @foreach($carts as $cart)
                    <div class="d-sm-flex justify-content-between align-items-center my-2 pb-3 border-bottom">
                        <div class="d-block d-sm-flex align-items-center text-center text-sm-start"><a class="d-inline-block flex-shrink-0 mx-auto me-sm-4" href="shop-single-v1.html"><img src="img/shop/cart/01.jpg" width="160" alt="Product"></a>
                            <div class="pt-2">
                                <h3 class="product-title fs-base mb-2"><a href="shop-single-v1.html">{{$cart->product_title}}</a></h3>
                                <div class="fs-sm"><span class="text-muted me-2">store:</span>{{$cart->shop->shop_name}}</div>
                                <div class="fs-sm"><span class="text-muted me-2">category:</span>{{$cart->product->category->category_name}}</div>
                                <div class="fs-lg text-accent pt-2">${{$cart->price}}</div>
                            </div>
                        </div>
                        <div class="pt-2 pt-sm-0 ps-sm-3 mx-auto mx-sm-0 text-center text-sm-start" style="max-width: 9rem;">
                            <label class="form-label" for="quantity1">Quantity</label>
                            <input class="form-control" disabled type="number" id="quantity1" min="1" value="{{$cart->quantity}}"> <form action="/cart/{{$cart->id}}/delete" method="post">
                                @csrf
                                @method('DELETE')
                ...
                <a class="btn btn-outline-accent d-block w-100 mt-4" href="/cart"><i class="ci-loading fs-base me-2"></i>Update cart</a>
                ...
                <div class="bg-white rounded-3 shadow-lg p-4">
                    <div class="py-2 px-xl-2">
                        <div class="text-center mb-4 pb-3 border-bottom">
                            ...
                            <h3 class="fw-normal">${{$totalprice}}</h3>
                        </div>
                        <div class="mb-3 mb-4">
                       ...
                       {{-- <div class="accordion" id="order-options">
                        ...
                        <form action="/checkout" method="post">
                            @csrf
                            <button class="btn btn-primary btn-shadow d-block w-100 mt-4">Pay on Delievery</button>
                        </form>
                        {{--<form action="/checkout/paywithcard">

                        </form>--}}
                        <a  class="btn btn-danger btn-shadow d-block w-100 mt-4" href="/stripe/{{$totalprice}}"><i class="ci-card fs-lg me-2"></i>Pay with Card</a>
                    </div>
                </div>
            </aside>
        </div>
